...
const BOARD = new Chess(PROMOTION_POSITION)
...
const onPieceDrop = ({
  ...
  function atMaterialDeficit(): boolean {
    return false;
  }
  ...
  function botMove() {
    do {
      ...
      BOARD.move(move)
      if (!atMaterialDeficit()) {
        break;
      }
      if (BOARD.inCheck()) {
        BOARD.undo()
      }
      BOARD.setTurn((playerColor === WHITE? "b": "w"))
    ...
    // Bot cannot make a move that would put opponent in check. Or else its insta lose!
